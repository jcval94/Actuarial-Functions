...
#Buscamos también la distribución del costo promedio anual. Si el ejercicio se pasa a
...
#Suponemos que la tasa de crecimiento del costo de la vivienda es X% anual
#Suponemos que el costo de los medicamentos/equipo/salario de quien los atiende, etc crece X% anual
#Y suponemos que nuestra población aumenta su edad de supervivencia con una tasa X% anual
...
Costo1<-sum(Anual[1]*Pond_Fin[1:Perm[1]])

#Repliquemos para nuestras 1000 personas
Costo<-purrr::map_dbl(1:1000,~sum(Anual[.x]*Pond_Fin[1:Perm[.x]]))
...
Valor_Presente1<-sum(Anual[1]*(Pond_Fin/Pond_Vivienda)[1:Perm[1]])

VP_Costos<-purrr::map_dbl(Perm,~sum(Anual[.x]*(Pond_Fin/Pond_Vivienda)[1:Perm[.x]]))
